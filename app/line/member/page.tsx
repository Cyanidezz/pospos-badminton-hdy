...
export default function LineMemberPage(){
  ...
  // Opened from "ยืนยันเบอร์นี้" / "สมัครสมาชิกด้วยเบอร์นี้" in the chat: that number's form comes first, so the card of
  // a number already on this account isn't mistaken for the one just looked up.
  const addCard=<section className="lm-card"><h2>{prefill?`เพิ่มเบอร์ ${formatPhone(prefill)}`:'เพิ่มเบอร์โทร'}</h2><p className="lm-muted">{prefill?'กรอกชื่อ และเลขรับไม้ถ้ามี เพื่อยืนยันว่าเป็นเบอร์ของคุณ แล้วจะเห็นดาวสะสมและรับแจ้งสถานะไม้ของเบอร์นี้ใน LINE':'เช่น เบอร์ของลูกที่มาขึ้นเอ็นกับร้าน จะได้ดูดาวสะสมและรับแจ้งสถานะไม้ของเบอร์นั้นด้วย'}</p><RegisterForm token={token} suggestedPhone={prefill} submitLabel={prefill?'ยืนยันเบอร์นี้':'เพิ่มเบอร์นี้'} onDone={done} onCancel={()=>{setAdding(false);setPrefill('')}}/></section>;
  return <main className="lm-page">
    ...
    {phones.length>0&&adding&&prefill&&addCard}
