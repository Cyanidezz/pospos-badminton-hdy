...
// while an ACTIVE promotion, the "ราคาขึ้นเอ็น" setting or an active product (a chatbot price card) actually uses
// that file, so this can't be used to read any
// other upload.
...
export async function GET(_req:Request,{params}:any){
  try{
    ...
    const f:any=(ready?.t?await one('SELECT f.mime FROM promotions p JOIN files f ON f.id=p.image WHERE p.image=? AND p.active=1 LIMIT 1',fileId):null)
      ...
      ||await one('SELECT f.mime FROM products p JOIN files f ON f.id=p.image WHERE p.image=? AND p.active=1 LIMIT 1',fileId);
